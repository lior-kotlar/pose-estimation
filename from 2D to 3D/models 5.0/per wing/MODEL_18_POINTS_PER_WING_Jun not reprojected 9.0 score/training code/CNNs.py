...
class Encoder2DAtrous(nn.Module):
    ...
    def get_conv2d(self, num_filters, input_channels):
        conv = nn.Conv2d(in_channels=input_channels, out_channels=num_filters,
                         kernel_size=self.kernel_size, padding=self.padding, dilation=self.dilation_rate)
...
class FourCamerasDisentanglement(nn.Module):
    def __init__(self, config, image_size, number_of_output_channels):
        ...
        self.rearrange_layer_1 = nn.Conv2d(in_channels=self.shared_encoder.get_output_size()[-1],
                                   out_channels=300,
                                   kernel_size=1,
                                   padding=0)
        self.FTL = FTL()
        ...
        self.rearrange_layer_2 = nn.Conv2d(in_channels=300,
                                   out_channels=self.shared_encoder.get_output_size()[-1],
                                   kernel_size=1,
                                   padding=0)
