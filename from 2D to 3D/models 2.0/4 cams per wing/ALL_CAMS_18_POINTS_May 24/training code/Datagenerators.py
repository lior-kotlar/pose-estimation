...
class DataGenerator:
    def __init__(self, config, preprocessor):
        ...
        self.train_dataloader = DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True,
                                           drop_last=False)
        self.val_dataloader = DataLoader(self.val_dataset, batch_size=2)
    ...
    def __getitem__(self, idx):
        ...
        if self.do_augmentations:
            if self.model_type == ALL_CAMS_18_POINTS:
                ...
                sample_box = np.concatenate((box1, box2, box3, box4), axis=-1)
                sample_confmaps = np.concatenate((confs1, confs2, confs3, confs4), axis=-1)
